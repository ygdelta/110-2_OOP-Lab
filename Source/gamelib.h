...
#define SIZE_X				 1600		// 設定遊戲畫面的解析度為1600x900
...
public:
	...
	void  OnShow();					// 將動畫貼到螢幕
	void  OnShow(double factor);	// 將動畫貼到螢幕(依比例放大)	#自行定義的函式
	void  Reset();					// 重設播放順序回到第一張圖形
	void  SetDelayCount(int);		// 設定動畫播放速度的常數(越大越慢)
	void  SetTopLeft(int,int);		// 將動畫的左上角座標移至 (x,y)
	int   Top();					// 取得動畫的左上角的 y 座標
	int   Width();					// 取得動畫的寬度
	int   getFirstBmpWidth();		// 取得第一張動畫寬度			#自行定義的函式
	int   getFirstBmpHeight();		// 取得第一張動畫高度			#自行定義的函式
private:
