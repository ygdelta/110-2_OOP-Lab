...
int CAnimation::getFirstBmpWidth() 
{
	GAME_ASSERT(bmp.size() != 0, "CAnimation: Bitmaps must be loaded first.");
	return bmp.begin()->Width();
}

int CAnimation::getFirstBmpHeight()
{
	GAME_ASSERT(bmp.size() != 0, "CAnimation: Bitmaps must be loaded first.");
	return bmp.begin()->Height();
}
...
void CGameState::ShowInitProgress(int percent)
{
	...
	pDC->SelectObject(pb);						// 釋放 brushgameStateTable
